...
export class CartService {
  ...
  private _getId(p: any): string | null {
    const cand = [p?.idProducto, p?.id, p?.productoId, p?.productoID, p?._id, p?.codigo, p?.slug];
  ...
  async confirmarCompra() {
    ...
    // ⛔ acá entra cuando NO hay stock, o cualquier otro error del backend
    const msg = data?.mensaje || `Error al confirmar compra ❌ (Código ${res.status})`;
    throw new Error(msg);
  }
}
